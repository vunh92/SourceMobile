...
class OutletDetailViewPage extends StatefulWidget {
  static const route = '/outlet_detail_view';

  const OutletDetailViewPage({Key? key}) : super(key: key);

  @override
  State<OutletDetailViewPage> createState() => _OutletDetailViewPageState();
}

class _OutletDetailViewPageState extends State<OutletDetailViewPage> {
 ...
 var outletPicture="";
  ...
  void initState() {
    ...
    var checkout=outletDetailController.checkOutResponse;
              ...
              Row(
                children: [
                  if(homeController.outletModel.value.currentStatus != null && homeController.outletModel.value.currentStatus!.isNotEmpty)
                    Text('at ${homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKIN.name()
                  ...
                  Container(
                    decoration: BoxDecoration(
                      color: homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKIN_ERROR.name()
                          ? AppColor.errorColor
                          : homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKIN.name()
                          ? AppColor.inProgressColor
                          : homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKOUT.name()
                          ? AppColor.checkedColor
                          : AppColor.unCheckedColor,
                      borderRadius: BorderRadius.circular(NumberConstant.baseRadiusBorderSmall),
                    ),
                    padding: const EdgeInsets.all(NumberConstant.basePaddingSmall),
                    child: Text(homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKIN_ERROR.name()
                        ? AppString.wrongLocation
                        : homeController.outletModel.value.currentStatus == CheckInStatusEnum.CHECKIN.name()
  ...
  Widget _stepsWidget(BuildContext context) {
    if(outletDetailController.steps.isEmpty || outletDetailController.checkOutResponse.currentStatus=="") return const SizedBox();
    return Stack(
      children: [
        Container(
        ...
        Column(
          children: [
            _stepRowWidget(
              isChecked:outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
              step: '1',
              icon: assetIconsPath + iconQrcodeSvg,
              name: AppString.scanQrcode,
              isRequired: true,
              enable: outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
            ),
            _stepRowWidget(
              isChecked:outletDetailController.checkOutResponse.checkOutText=="Thất lạc" || outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
              step: '2',
              icon: assetIconsPath + iconImageSvg,
              name: AppString.hotZone,
              isRequired: outletDetailController.checkOutResponse.outletHotZoneCheck==1,
              enable: outletDetailController.checkOutResponse.checkOutText=="Thất lạc" || outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
            ),
            // _stepRowWidget(
            //   isChecked: outletDetailController.checkOutResponse.checkOutText=="Thất lạc" || outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
            //   step: '3',
            ...
            //   isRequired: outletDetailController.checkOutResponse.outletStockCountCheck==1,
            //   enable: outletDetailController.checkOutResponse.checkOutText=="Thất lạc" || outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,
            // ),
            _stepRowWidget(
              isChecked: outletDetailController.checkOutResponse.checkOutText=="Thất lạc" || outletDetailController.checkOutResponse.coolerStatus=="Chưa thiết lập"?false: true,

...
class _Constant {
  ...
  static title(int number) => 'Chưa viếng thăm $number cửa hàng';
}
...
class _UnCheckedScreenState extends State<UnCheckedScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<HomeController>(
        ...
        builder: (controller) {
          return Obx(() {
            if(homeController.unCheckedOutlets.isEmpty) {
              return _emptyWidget();
            }
            if(homeController.searchUncheckedOutlets.isNotEmpty) {
              return _buildBody(homeController.searchUncheckedOutlets);
            }
            return _buildBody(homeController.unCheckedOutlets);
          });
        }
    );
  }
